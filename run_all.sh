...
usage() {
    ...
    echo "  Alternative approach: Use LLM to generate comprehensive profiles, then chunk"
    echo "  11 - Create LLM profile table"
    echo "  12 - Generate comprehensive LLM profiles (using TEXT_GENERATION)"
    echo "  13 - Chunk LLM profiles (alternative to step 6-7)"
    echo "  14 - Generate embeddings for LLM chunks"
    echo "  15 - Test LLM semantic search"
    echo ""
    echo "  EXERCISE: Compare the two chunking approaches:"
    echo "    • Steps 6-7: Direct chunking of JSON documents"
    echo "    • Steps 12-13: LLM-generated profiles, then chunked"
    echo "    Run the same semantic search queries on both and compare results!"
    echo ""
    echo "Examples:"
